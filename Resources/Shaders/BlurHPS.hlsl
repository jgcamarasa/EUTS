...
float4 main(PixelInput input) : SV_TARGET
{
	...
	float4 c0 = shaderTexture.Sample(samplerState, input.texCoord-float2(-2.0*pW, 0.0f));
		float4 c1 = shaderTexture.Sample(samplerState, input.texCoord - float2(-1.0*pW, 0.0f));
