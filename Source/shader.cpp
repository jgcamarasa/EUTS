...
void EUTS_Shader_ColorTextured_initialize(EUTS_Shader *shader, EUTS_RenderState *renderState, WCHAR *vsFilename, WCHAR *psFilename)
{
	HRESULT result;
	ID3DBlob *errorMessage;
	ID3DBlob *vertexShaderBuffer;
	...
	// What to do here? Where do we store the sampler state for this shader? 
	// A struct for each shader? For each type of shader? (Shader with texture, shader without texture)
	// Just one struct with sampler, etc, and only using the one needed? (not sure about this one...)
	//result = renderState->device->CreateSamplerState(&samplerDesc, &) 
	assert(false); // Unimplemented
}

void EUTS_Shader_ColorTextured_setParameters(EUTS_Shader *shader, EUTS_RenderState *renderState, XMMATRIX *modelMatrix, XMMATRIX *viewMatrix, XMMATRIX *projectionMatrix)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	EUTS_VSMatrixConstantBuffer *dataPtr;
